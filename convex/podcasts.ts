...
export const getPodcastBySearch = query({
  ...
  handler: async (ctx, args) => {
    if (args.search === "") {
      // Return all podcasts ordered by latest if search is empty
      return await ctx.db.query("podcasts").order("desc").collect();
    }
    ...
    // If results are found by author, return them
    if (authorSearch.length > 0) {
      return authorSearch;
    }

    // If no author matches, search by podcast title
    const titleSearch = await ctx.db
      .query("podcasts")
      .withSearchIndex("search_title", (q) => q.search("podcastTitle", args.search))
      .take(10);

    // If results are found by title, return them
    if (titleSearch.length > 0) {
      return titleSearch;
    }

    // Finally, search by podcast description if no matches by author or title
